...
for branch in "${branches[@]}"; do
	...
	for dir in "${directories[@]}"; do
		...
		# TODO remove this whole case statement when "graal" directories are removed from the "6" branch?  leave as a harmless safety net against mis-tagging or version scraping mishaps?  (perhaps turn into an error instead of a warning 👀)
		case "$version" in
			"$major".*) ;;
			*) echo >&2 "warning: skipping $dir on $branch (version $version is not $major.x)"; continue ;;
		esac
		...
		if [ "$version" != "$firstVersion" ]; then
			...
			echo >&2 "warning: $dir on $branch contains $version (compared to $firstVersion in ${directories[0]})"
		fi
		...
		if [ "$jdk" = 'jdk20' ]; then
			# TODO delete the offending "jdk20*" directories from 7 and 6 branches and remove this conditional
			echo >&2 "warning: skipping $dir on $branch (contains $jdk which is unsupported)"
			continue
		fi
		...
		if [ "$jdk" = 'jdk-lts-and-current' ]; then
			# TODO actually scrape java versions from $dockerfile (complicated for graal)
			case "$variant" in
				graal)
		...
		if [ "$jdk" = 'jdk-lts-and-current' ] && [ "$variant" != 'graal' ]; then
			# *technically*, we could re-use "currentFrom" that we scraped above, but there's a lot of conditional logic betwen here and there so it's safer to just re-scrape it
			...
			if [ "$arches" != "$copyFromArches" ]; then
				...
				echo >&2 "error: arches mismatch betwen $from and $copyFrom in $dir on branch $branch ('$arches' vs '$copyFromArches')"
				exit 1
			fi
		fi
